...
def expected_improvement(X, X_sample, Y_sample, kriging, xi=0.01):
    ...
    mu_sample_opt = np.min(kriging.predict_values(X_sample))
...
def propose_location(acquisition, X_sample, Y_sample, kriging, bounds, n_restarts=25):
    ...
    def min_obj(X):
        ...
        return -acquisition(X.reshape(-1, dim), X_sample, Y_sample, kriging).reshape(-1)

    for x0 in np.random.uniform(bounds[:, 0], bounds[:, 1], size=(n_restarts, dim)):
        res = minimize(min_obj, x0=x0, bounds=bounds, method='L-BFGS-B')
        # 如果res.fun是一个float类型的值
        if type(res.fun) == float:
            if res.fun < min_val:
                min_val = res.fun
                min_x = res.x
        else:
...
# 随机选择初始采样点
X_sample = np.random.uniform(-15, 30, size=(100, 2))

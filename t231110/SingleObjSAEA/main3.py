...
# phase2
surrogate_model_type = "rbf" # 代理模型名称，可选代理模型见surrogate.py
...
flag = (nFE - alg_args["n_init_sample"]) / 2
while nFE < alg_args["max_nFE"]:
    ...
    if epoch < flag:
        ...
        # 2.1 创建代理模型并训练
        surrogate_model2 = surrogate(print_training=False)
        surrogate_model2.set_training_values(X, y)
        surrogate_model2.train()
        # print(surrogate_model.predict_values(X))
        problem_predicted_by_surrogate = p.get_problem(setting=problem_setting1, surrogate=surrogate_model2)
        # print(problem_predicted_by_surrogate.evaluate(X))
